...
function writeFiles(folder, path, obj, i, props) {
    ...
    path.methods.forEach(method => {
        props.method = method
        obj.template('riverpod.operation.form.ejs',`${pathFolder}/${method.operationId}.dart`, obj, props)

        obj.template('riverpod.operation.bloc.ejs', `${storeFolder}/${method.operationId}_bloc.dart`, obj, props)
    })
}